...
- Innovation: Upcoming large-scale surveys are expected to uncover large numbers of LSBGs, requiring robust automated methods to identify them across heterogeneous datasets.
- Authors: Hareesh Thuruthipilly, Krzysztof Lisiecki, Junais, Katarzyna Małek, Agnieszka Pollo, William J. Pearson, Antonio Vanzanella, Saptarshi Pal
- Year: 2026
- URL: http://arxiv.org/abs/2605.13842v1
...
- Score: 20.0
- Reason: recent publication
- Innovation: Based on the above observation, we propose SPA (Semantic-guided Patch-level Alignment) for CLIP-based CIL, which aims to awaken long-neglected local representations within CLIP. Building upon these selected patches, we further employ optimal transport to align selected patch tokens with semantic tokens from class-wise descriptions, yielding a structured cross-modal alignment that improves recognition.
- Authors: Hao Sun, Zi-Jun Ding, Da-Wei Zhou
- Year: 2026
- URL: http://arxiv.org/abs/2605.13835v1
- Matched queries: semi-supervised medical image segmentation, weakly supervised medical image segmentation, pseudo-label medical image segmentation
...
## 3. QLAM: A Quantum Long-Attention Memory Approach to Long-Sequence Token Modeling

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: In this work, we introduce one of the first studies to leverage the superposition property of quantum systems to enhance state-based sequence modeling. In particular, we propose Quantum Long-Attention Memory (QLAM), a hybrid quantum-classical memory mechanism that can be viewed as a quantum extension of state-space models.
...
## 4. On the Lie Foliation structure of Walker Manifolds

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: We study Walker manifolds, that is, pseudo-Riemannian manifolds $(M^n,g)$ admitting a null parallel distribution $\D$ of rank $r\leq\frac{n}{2}$. We show that $\D$ always integrates to a $G$-Lie foliation $\F_\D$, where $G$ is the simply connected Lie group with Lie algebra equal to the structure algebra $\g_\D$ of $\D$.
- Authors: Ameth Ndiaye
- Year: 2026
- URL: http://arxiv.org/abs/2605.13820v1
- Matched queries: semi-supervised medical image segmentation, weakly supervised medical image segmentation, pseudo-label medical image segmentation
...
- Score: 20.0
- Reason: recent publication
- Innovation: Over an extensive experimental evaluation on open-source hemodialysis safety requirements, we show that the LLM-based approach in VERIMED successfully reduces ambiguity-sensitive requirements and enables rigorous auditing of software requirements through SMT-based queries.
...
## 6. OmniLiDAR: A Unified Diffusion Framework for Multi-Domain 3D LiDAR Generation

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: To this end, we present OmniLiDAR, a unified text-conditioned diffusion framework that generates LiDAR scans in a shared range-image representation across eight representative domains spanning three shift types: adverse weather, sensor-configuration changes (e.g., reduced beams), and cross-platform acquisition (vehicle, drone, and quadruped). To enable training a single model over heterogeneous domains without isolating optimization by domain, we introduce a Cross-Domain Training Strategy (CDTS) that mixes domains within each mini-batch and leverages conditioning to steer generation.
- Authors: Youquan Liu, Weidong Yang, Ao Liang, Xiang Xu, Lingdong Kong, Yang Wu, Dekai Zhu, Xin Li
- Year: 2026
- URL: http://arxiv.org/abs/2605.13815v1
...
## 8. VoxCor: Training-Free Volumetric Features for Multimodal Voxel Correspondence

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: We introduce VoxCor, a training-free fit--transform method for reusable volumetric feature representations from frozen 2D ViT foundation models. VoxCor improves the hardest cross-subject, cross-modality transfer settings, reduces encoder sensitivity for dense correspondence transfer, and yields registration performance competitive with handcrafted descriptors and learned 3D features.
- Authors: Guney Tombak, Ertunc Erdil, Ender Konukoglu
- Year: 2026
- URL: http://arxiv.org/abs/2605.13798v1
...
## 9. LMPath: Language-Mediated Priors and Path Generation for Aerial Exploration

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: We also ran simulations to demonstrate how paths generated using LMPath outperform traditional path planning approaches for search missions.
...
- Score: 20.0
- Reason: recent publication
- Innovation: To bridge this gap, we introduce RoboEvolve, a novel framework that couples a VLM planner and a VGM simulator into a mutually reinforcing co-evolutionary loop.
- Authors: Harold Haodong Chen, Sirui Chen, Yingjie Xu, Wenhang Ge, Ying-Cong Chen
- Year: 2026
- URL: http://arxiv.org/abs/2605.13775v1
- Matched queries: semi-supervised medical image segmentation, scribble supervised segmentation, pseudo-label medical image segmentation
