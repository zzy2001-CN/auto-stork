# Literature Digest 2026-07-15

Profile: `default`
Total recommendations: 10

## 1. Open-KNEAD: Knowledge-grounded Nutrition Estimation via Agentic Decomposition

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: This raises a question: if retrieval no longer improves the overall estimate, can it still deliver the two things clinicians value, accurate portions and a traceable, item-by-item record? We introduce Open-KNEAD, a knowledge-grounded agentic framework for meal nutrition estimation that is training-free and locally deployable.
...
## 2. Rank-1 Identity Consensus Predicts Gallery Enrollment in 1:N Face Matching More Accurately than Score Thresholding

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: Most approaches threshold match scores, but scores shift substantially with image quality and gallery size and composition, making thresholds fixed before deployment brittle under realistic conditions. Our prior work introduced 1-consistency, the only method based on rank consensus across multiple independently trained matchers: a probe is labeled MP if all matchers return the same rank-1 identity.
- Authors: Gabriella Pangelinan, Aman Bhatta, Michael C. King, Kevin W. Bowyer
- Year: 2026
- URL: http://arxiv.org/abs/2607.12903v1
- Matched queries: semi-supervised medical image segmentation, weakly supervised medical image segmentation, pseudo-label medical image segmentation
...
## 3. Regular Curves, Singular Graphs: Cantor Parts and the Relaxed Willmore Energy

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: One might expect that finite relaxed elastic energy rules out diffuse singularities in the derivative, leaving only absolutely continuous and jump parts. This is suggested by the role of $SBV$ in free-discontinuity problems and by interpreting jumps as vertical segments of limiting graphs.
- Authors: Hans-Christoph Grunau, Boris Gulyak
- Year: 2026
- URL: http://arxiv.org/abs/2607.12899v1
...
## 4. Astra: an open-source fully autonomous robotic observatory control software

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: Robotic and autonomous observatories are critical for modern time-domain and high-cadence astronomical surveys. The operation of these facilities requires complex software coordination to manage hardware, schedule observations, and ensure safety.
- Authors: Peter P. Pedersen, David Degen, Lionel Garcia, Urs Schroffenegger, Daniel Sebastian, Sebastián Zúñiga-Fernández, Brice-Olivier Demory, Elsa Ducrot
- Year: 2026
- URL: http://arxiv.org/abs/2607.12898v1
- Matched queries: semi-supervised medical image segmentation, weakly supervised medical image segmentation, scribble supervised segmentation, pseudo-label medical image segmentation
...
## 5. UniMedSeg: Unified In-Context Learning for Multi-Paradigm 2D/3D Medical Image Segmentation

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: Medical image segmentation foundation models are expected to generalize across diverse clinical scenarios, yet existing universal methods remain fragmented by prompt paradigms and spatial dimensions. To address this bottleneck, we propose UniMedSeg, a Transformer-centric universal segmentation framework that maps visual examples, geometric interactions, language instructions, and 2D/3D images into a shared sequence space, enabling heterogeneous medical supervision to be jointly learned through a unified in-context interface without prompt- or dimension-specific branches.
...
- Score: 20.0
- Reason: recent publication
- Innovation: Yet current evaluation frameworks do not capture model behavior in these settings, where misconceptions can emerge, persist, or evolve over the course of a conversation. To study this, we introduce ThReadMed-QA, a multi-turn medical dialogue dataset of 2,437 patient-physician conversation threads comprising 8,204 question-answer pairs, derived from real patient interactions on AskDocs.
...
## 7. Metric-Guided Synthetic Image Data Rendering for Deep Learning compatible with Agentic AI

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: We present GraNatPy, a Python package with metrics to guide improvement of the rendered scene. We show that quantifiable increase in realism, diversity and size of rendered dataset correlates with improved visual perception of the scene and higher zero-shot performance of an object detection model.
- Authors: Martina Radoynova, Samuel Pantze, Trina De, Ulrik Günther, Artur Yakimovich
- Year: 2026
- URL: http://arxiv.org/abs/2607.12874v1
- Matched queries: semi-supervised medical image segmentation, weakly supervised medical image segmentation, pseudo-label medical image segmentation
...
## 9. LARAD: Layout-Aware Road Anomaly Detection via Spatial-Logic Reasoning

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: Current anomaly segmentation methods suffer from a fundamental blind spot: they over-rely on texture novelty to identify out-of-distribution (OoD) objects while ignoring contextual spatial logic. To address these issues, we propose Layout-Aware Road Anomaly Detection (LARAD), shifting the paradigm from appearance matching to spatial-logic reasoning.
- Authors: Shiyi Mu, Xujie Chen, Shugong Xu
- Year: 2026
- URL: http://arxiv.org/abs/2607.12858v1
...
## 10. The MICADO first light imager for the ELT: the SCAO module AIT in France, from the beta flat configuration towards the flat and final configurations

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: In this contribution I will first present the progress made in the AIT of the beta flat configuration and its closed loop results.
- Authors: Yann Clénet, Vincent Lapeyrère, Eric Gendron, Fabrice Vidal, Frédéric Chapron, Mathieu Cohen, Sylvain Guieu, Roderick Dembet
- Year: 2026
- URL: http://arxiv.org/abs/2607.12854v1
