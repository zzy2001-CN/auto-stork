...
- Innovation: Naively removing it introduces a quality gap, as the model must learn both high-level structure and low-level details from raw pixels. In this paper, we propose Representation Forcing (RF), a technique that closes this gap by making representation prediction a native capability of the model.
- Authors: Yuqing Wang, Zhijie Lin, Ceyuan Yang, Yang Zhao, Fei Xiao, Hao He, Qi Zhao, Zihan Ding
- Year: 2026
...
- Score: 20.0
- Reason: recent publication
- Innovation: To enable a systematic SC evaluation, we introduce SOCO, a new benchmark for Semantic Object Correspondence that introduces a taxonomy of correspondence types and provides consistent, functionally meaningful keypoint annotations across 100 categories and over 1M correspondence pairs.
...
- Score: 20.0
- Reason: recent publication
- Innovation: However, existing approaches to OOD detection often require some knowledge of the shifted distribution, fail to detect subtle or localized distribution shifts, and operate on full images, rather than the indirect measurements available in inverse problems. We propose an OOD detection metric based on the Kullback-Leibler divergence between the diffusion prior and the posterior distribution, that (i) does not require any calibration data or knowledge of the shifted distribution, and (ii) can detect whole images as OOD as well as localize OOD patches within an image.
...
## 4. CoFiDA-M: Concept-Aware Feature Modulation for Cross-Domain Adaptation with Image-Only Inference

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: Existing domain adaptation methods often ignore crucial semantic invariants, such as clinical concepts. We address this gap by proposing CoFiDA-M, a privileged information framework that learns from concepts at training time but deploys as an image-only model.
...
- Score: 20.0
- Reason: recent publication
- Innovation: We introduce nuReasoning, a large-scale real-world dataset and benchmark for reasoning-centric AD. Experiments show that fine-tuning VLMs on nuReasoning substantially improves driving-specific question answering, while incorporating reasoning supervision into VLA training improves planning performance even when textual reasoning outputs are disabled at inference time.
- Authors: Zhiyu Huang, Johnson Liu, Rui Song, Zewei Zhou, Ruining Yang, Yun Zhang, Tianhui Cai, Hanyin Zhang
- Year: 2026
...
## 6. Functional Attention: From Pairwise Affinities to Functional Correspondences

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: These methods treat continuous fields as discrete tokens and usually ignore the global functional structure. We introduce \emph{Functional Attention}, which reinterprets attention as a functional correspondence between adaptive bases.
- Authors: Jiefang Xiao, Maolin Gao, Simon Weber, Guandao Yang, Daniel Cremers
- Year: 2026
- URL: http://arxiv.org/abs/2605.31559v1
...
## 7. Vision-Language Models Suppress Female Representations Under Ambiguous Input

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: We introduce LALS (Latent Association Leaning Score), a zero-shot metric that projects visual-token activations into the model's text-embedding space to measure concept associations per token and layer.
...
## 8. Automated Prediction of Postoperative Pancreatic Fistula Using Preoperative Computed Tomography

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: This approach offers a clinically valuable tool and a methodological benchmark for pancreas-specific CT classification, supporting improved preoperative decision-making in pancreatic surgery.
...
- Year: 2026
- URL: http://arxiv.org/abs/2605.31539v1
- Matched queries: semi-supervised medical image segmentation, scribble supervised segmentation, pseudo-label medical image segmentation
...
## 10. BERS: Locally Optimal Continuous Algorithm for Maritime Weather Routing with Just-in-Time Arrival

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: We present Bézier Evolve and Refine Strategy (\name{}), a two-stage framework that combines global evolutionary search (CMA-ES) with local variational refinement (FMS). Across these tests, \name{} matches or improves published baselines while maintaining robust convergence under challenging flow fields and land geometries.
- Authors: Daniel Precioso, Francisco Suárez, Javier Jiménez de la Jara, Rafael Ballester-Ripoll, David Gómez-Ullate
- Year: 2026
- URL: http://arxiv.org/abs/2605.31533v1
- Matched queries: semi-supervised medical image segmentation, pseudo-label medical image segmentation
