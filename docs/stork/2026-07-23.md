# Literature Digest 2026-07-23

Profile: `default`
Total recommendations: 10

## 1. Near-Optimal Dimension Lower Bounds for Single-Vector Embeddings of Maximum Inner Product Similarity

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: Since $δ$ can be arbitrarily small, the exponent approaches the $O(1/\varepsilon^2)$ dependence of the upper bound. The proof combines Sherstov's pattern matrix method with polynomial-size, constant-width DNF formulas computing functions of approximate degree $Ω(k^{1-δ})$.
- Authors: Rajesh Jayaram, Honghao Lin, Vahab Mirrokni, David P. Woodruff
- Year: 2026
- URL: http://arxiv.org/abs/2607.20393v1
...
- Score: 20.0
- Reason: recent publication
- Innovation: A unified MRN-UUIDv7 identifier framework with QR-based tracking ensures traceable clinical-research linkage under PHI residency constraints. Deployment demonstrates improved workflow observability, reduced QC latency, and enhanced cross-role transparency between laboratory technicians, research coordinators, and patient-facing teams.
...
## 4. The High-Speed FPGA Readout System for the Advanced X-ray Imaging Satellite (AXIS)

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: Our design demonstrates the goal readout performance for AXIS (20 frames per second) and provides a framework for future high-throughput X-ray observatories.
- Authors: Declan O'Neill, Jill Juneau, Peter Orel, Sven Herrmann, Gregory Prigozhin, Eric D. Miller, Steven W. Allen, Marshall W. Bautz
- Year: 2026
- URL: http://arxiv.org/abs/2607.20380v1
- Matched queries: semi-supervised medical image segmentation, weakly supervised medical image segmentation, pseudo-label medical image segmentation
...
## 5. The burgeoning rise of the 21-cm forest I: constraints on the optical depth of the intergalactic medium at $5.38 < z < 5.84$

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: The redshifted 21-cm line is a promising probe of the Epoch of Reionization, during which the first generation of stars ionized the InterGalactic Medium (IGM). We aimed to constrain the IGM neutral Hydrogen fraction and spin temperature via redshifted 21-cm absorption against high-redshift radio sources.
- Authors: Chanasorn Kongprachaya, Gianni Bernardi, Emilio Ceccotti, Andrei Mesinger, Benedetta Ciardi, Oleg M. Smirnov
- Year: 2026
- URL: http://arxiv.org/abs/2607.20371v1
- Matched queries: semi-supervised medical image segmentation, weakly supervised medical image segmentation, pseudo-label medical image segmentation
...
## 6. Relative smooth surgery structure sets of thickenings of the Cayley projective plane and applications

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: We compute the relative smooth surgery structure sets of the thickenings $\mathbb{OP}^{2}\times\mathbb{D}^{k}$ of the Cayley projective plane $\mathbb{OP}^{2}$ for every $k\geq 1$ with $k\equiv 0\pmod 4$, by determining the corresponding normal invariants and surgery obstruction map. We show that the latter is not surjective and determine the $2$-adic valuation of the generator of its image.
- Authors: Souvik Mandal, Ankur Sarkar
- Year: 2026
- URL: http://arxiv.org/abs/2607.20362v1
- Matched queries: semi-supervised medical image segmentation, weakly supervised medical image segmentation, pseudo-label medical image segmentation

<details>
<summary>Abstract</summary>

We compute the relative smooth surgery structure sets of the thickenings $\mathbb{OP}^{2}\times\mathbb{D}^{k}$ of the Cayley projective plane $\mathbb{OP}^{2}$ for every $k\geq 1$ with $k\equiv 0\pmod 4$, by determining the corresponding normal invariants and surgery obstruction map. We show that the latter is not surjective and determine the $2$-adic valuation of the generator of its image. As applications, we construct infinitely many pairwise non-homeomorphic closed smooth manifolds of dimension $16+k$, homotopy equivalent to $\mathbb{OP}^{2}\times\mathbb{S}^{k}$ and distinguished by their total Pontryagin numbers; we compute the rational homotopy groups of the block diffeomorphism group $\widetilde{\operatorname{Diff}}(\mathbb{OP}^{2})$ in every degree congruent to $3$ modulo $4$; and we construct smooth $\mathbb{OP}^{2}$-bundles over $\mathbb{S}^{4}$, $\mathbb{S}^{8}$, and $\mathbb{S}^{12}$ whose total spaces have non-vanishing $\widehat{\mathfrak{A}}$-genus. These bundles yield elements of infinite order in the homotopy groups of the spaces of metrics of positive sectional, Ricci, and scalar curvature on $\mathbb{OP}^{2}$ in degrees $3$, $7$, and $11$.

</details>

## 7. PolySim: Deterministic Polynomial Surrogates for Cross-Modal Retrieval on CiM

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: The core difficulty is a sampling gap: probabilistic methods such as PCME rely on Monte Carlo sampling and nonlinear distance evaluation at inference, which are fundamentally incompatible with CiM crossbar arrays that support only deterministic, single-step matrix-vector multiplication. Few existing probabilistic retrieval methods can be executed on a conventional crossbar.
- Authors: Xinzhao Li, Charles Power, Pengyu Ren, Jongun Won, Likai Pei, Yuting Hu, Jinjun Xiong, Alptekin Vardar
- Year: 2026
- URL: http://arxiv.org/abs/2607.20358v1
- Matched queries: semi-supervised medical image segmentation, weakly supervised medical image segmentation, pseudo-label medical image segmentation
...
## 9. Test-Time Training for Modality Order Consistency in Vision-Language Models

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: We use this gap to design an order-consistent test-time training method. Our method substantially closes the modality-order gap across all evaluated settings.
...
## 10. Toward Reliable RGB-D Semantic Segmentation: Handling Missing Modalities via Condition Dropout

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: Experiments on NYU-Depth V2 and SUN RGB-D show that ConD improves robustness under missing modalities and even yields slight gains when modalities are complete.
