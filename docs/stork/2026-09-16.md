...
- Innovation: We introduce ModAR, the first WAM to autoregressively denoise multiple future modalities before predicting actions. On three real-world bimanual tasks, ModAR outperforms baselines and improves with human videos.
- Authors: Adam Hung, Bardienus P. Duisterhof, Deva Ramanan, Jeffrey Ichnowski
- Year: 2026
- URL: http://arxiv.org/abs/2609.17524v1
...
- Score: 20.0
- Reason: recent publication
- Innovation: However, most prior methods either operate on single-codebook codecs or apply a single compression step before multi-layer quantization. We propose LACE (Layer-Adaptive Codec Encoding), a dynamic frame rate codec that applies an independent compression step at each quantization layer, enabling layer-specific segmentation boundaries.
...
## 4. Extending Andreotti's proof of Torelli's theorem to nodal curves

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: Let $C$ be a connected nodal curve of genus $g$ over an algebraically closed field $k$ of characteristic zero. Let \[ A(C)=\Big(J(C), {\overline{P^{g-1}_C}}, Θ(C) \Big) \] be the stable semi-abelic pair (in the sense of Alexeev) associated with its canonical compactified Jacobian $\overline{P^{g-1}_C}$ in degree $g-1$.
- Authors: Alex Abreu, Marco Pacini, Nicola Pagani
- Year: 2026
- URL: http://arxiv.org/abs/2609.17502v1
- Matched queries: semi-supervised medical image segmentation, weakly supervised medical image segmentation, pseudo-label medical image segmentation
...
## 5. First-Crossing Reduction and Inference for No-Rescue Effects under Deterministic Rescue

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: Clinical protocols may require rescue medication when a patient's condition crosses a prespecified threshold. The outcome under continued non-rescue is then unobserved after first crossing, preventing point identification without extrapolation assumptions.
- Authors: Shu Tamano
- Year: 2026
- URL: http://arxiv.org/abs/2609.17495v1
- Matched queries: semi-supervised medical image segmentation, weakly supervised medical image segmentation, pseudo-label medical image segmentation
...
- Year: 2026
- URL: http://arxiv.org/abs/2609.17480v1
- Matched queries: semi-supervised medical image segmentation, weakly supervised medical image segmentation, pseudo-label medical image segmentation
...
## 7. Tables Decoded: DELTA for Structure, TARQA for Understanding

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: While recent approaches predominantly rely on vision- language models (VLMs) operating on table images, we propose a more scalable and effective alternative based on structured textual representations. On table structure recognition (TSR), DELTA achieves TEDS- Structure scores comparable with state-of-the-art methods across FinTabNet, PubTabNet, and PubTables-1M.
...
## 8. Finite-Sample Hausdorff Bounds and Hadamard Sensitivity for Regressions with MNAR Covariates

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: We separately study a Hadamard-design method for exploring the effect of admissible imputations without enumerating all vertices and build intuition to detect situations where the population target can be approximated by a zonotope.
- Authors: Hugo Dunias
- Year: 2026
- URL: http://arxiv.org/abs/2609.17456v1
- Matched queries: semi-supervised medical image segmentation, weakly supervised medical image segmentation, pseudo-label medical image segmentation
...
## 9. ORCA: Occlusion-Aware Refinement and Completion for Novel View Synthesis

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: Novel-view synthesis from a single image is a fundamentally ambiguous problem. Existing methods often rely on generative models to complete such regions.
...
## 10. BrainFocus: EEG-Guided ROI Selection for Efficient Vision-Language Models

- Source: arXiv
- Score: 20.0
- Reason: recent publication
- Innovation: In this work, we propose BrainFocus, a reliable EEG-guided efficient VLM framework for VQA. Across Qwen3.5-VL 2B, 4B, and 9B models, BrainFocus improves VQA accuracy by 4.14-9.87 percentage points (pp) on cluttered scenes while reducing input tokens and total tokens by 23.2%-39.4% and 23.2%-39.3%, and end-to-end floating-point operations (FLOPs) by 23.2%-39.5%.
